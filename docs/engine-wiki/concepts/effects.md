...
- 净化只有一处（`effects.act_cleanse`，`effects.py:580-584`）
...
例 `schedule.py:196`、`stats.py:55`、`effects.py:459`）。

| 字段 | 谁写 | 谁读 | 含义 |
|---|---|---|---|
| `stacks` | `act_apply`（`effects.py:422/429`）/ `act_consume` / 周期 gain | `_cap_of` clamp、`stats` 折算、`schedule` 周期跳、`_apply_death_guard` | 层数。**允许 float**（小数刻度，如信仰每刻 −0.7） |
...
`act_apply` 的值型分支会写 `{"v": 0.45}`（`effects.py:449-451`），并给 `reduce` 额外写
`holder["reduce_left"]`（`effects.py:452-453`）。但：
...
（`effects.py:502`）、`schedule` 的 gain 分支 `round(..., 6)`（`schedule.py:406`）。

...
- 到期只有一处（`schedule._settle_time_effects`，`schedule.py:436-455`）
- 净化只有一处（`effects.act_cleanse`，`effects.py:625-634`）
...
例 `schedule.py:407`、`stats.py:54`、`effects.py:485`）。
...
| `expire` | `act_apply` | `schedule._settle_time_effects`（`schedule.py:416-420`）、`Battle.act` 控制过期兜底（`battle.py:469-471`）、`actions._consume_hit_buffs` | **绝对时刻**；`None` = 永不到期 |
...
| `period` | **内容侧**直接写入 | `schedule._settle_time_effects`（`schedule.py:478-486`） | 动态周期声明（条目自带优先，回落表声明） |
| `value` | 内容侧（`heal_amp_pct` 等） | `landing._apply_heal_mods`（`landing.py:518`） | 附加数值袋（形态自定，消费方自己解释） |

### `v` 字段的消费缺口

`act_apply` 的值型分支会写 `{"v": 0.45}`（`effects.py:472-474`），并给 `reduce` 额外写
`holder["reduce_left"]`（`effects.py:475-476`）。但：
...
| 函数 | 行为 |
|---|---|
| `_norm_stack(v)` / `norm_stack`（`effects.py:49`，S2 公开别名 `:80`） | 写回归一：整值 → `int`；小数 → `round(6)`（清 `10-0.7 → 9.3` 的二进制尾差） |
| `_fmt_stack(v)`（`effects.py:40`） | 日志显示：整值去掉 `.0`（`9.3` 显示成 `9.3`，`10.0` 显示成 `10`） |

**为什么需要 float**：资源可以有非整数速率（信仰每刻 −0.7、磐核每刻 +0.4）。
`apply` 的叠层分支读 `float()`（`effects.py:423`）、`consume` 读 `float()`
（`effects.py:523`）、`schedule` 的 gain 分支 `round(..., 6)`（`schedule.py:670`）。
读侧全用 `float()` 保真，写侧统一过 `norm_stack` 保持「int 资源看起来还是 int」。

## cap 的唯一收敛点

```python
def _cap_of(actor, key):                       # effects.py:61，S2 公开别名 cap_of
...
`schedule` 周期 gain 分支（`schedule.py:667`，且**表声明可被 `period.cap` 覆盖**）。
...
引擎的实现要点（`schedule._settle_time_effects`，`schedule.py:472-620`）：

1. **首次挂不给跳**：第一次看到某 key 时只登记 `dot_next[key] = now + interval`
   （`schedule.py:500-503`，对齐旧引擎的「首跳延迟」）
...
4. **`turns` 限跳**：跳够 `turns` 次就清层（计数器 `dot_jumps`，`schedule.py:671-677`）
5. **`dir="gain"` 不需要 `stacks > 0`**：0 层也要回（游侠精力耗到 0 若被拦将永远回不了，
   `schedule.py:490-493` 注释）

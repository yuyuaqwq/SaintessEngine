...
- 到期只有一处（`schedule._settle_time_effects`，`schedule.py:432-449`）
- 净化只有一处（`effects.act_cleanse`，`effects.py:612-621`）
...
例 `schedule.py:406`、`stats.py:54`、`effects.py:472`）。

| 字段 | 谁写 | 谁读 | 含义 |
|---|---|---|---|
| `stacks` | `act_apply`（`effects.py:430/429`）/ `act_consume` / 周期 gain | `_cap_of` clamp、`stats` 折算、`schedule` 周期跳、`_apply_death_guard` | 层数。**允许 float**（小数刻度，如信仰每刻 −0.7） |
| `expire` | `act_apply` | `schedule._settle_time_effects`（`schedule.py:412-416`）、`Battle.act` 控制过期兜底（`battle.py:455-457`）、`actions._consume_hit_buffs` | **绝对时刻**；`None` = 永不到期 |
| `mode` | `act_apply` 控制分支 | `Battle.act` 控制消费（`battle.py:464-491`） | `"skip"` = 整跳行动 / `"no_skill"` = 技能转普攻 |
...
| `period` | **内容侧**直接写入 | `schedule._settle_time_effects`（`schedule.py:475-483`） | 动态周期声明（条目自带优先，回落表声明） |
| `value` | 内容侧（`heal_amp_pct` 等） | `landing._apply_heal_mods`（`landing.py:487`） | 附加数值袋（形态自定，消费方自己解释） |

### `v` 字段的消费缺口

`act_apply` 的值型分支会写 `{"v": 0.45}`（`effects.py:459-461`），并给 `reduce` 额外写
`holder["reduce_left"]`（`effects.py:462-463`）。但：
...
| 函数 | 行为 |
|---|---|
| `_norm_stack(v)` / `norm_stack`（`effects.py:47`，S2 公开别名 `:80`） | 写回归一：整值 → `int`；小数 → `round(6)`（清 `10-0.7 → 9.3` 的二进制尾差） |
| `_fmt_stack(v)`（`effects.py:38`） | 日志显示：整值去掉 `.0`（`9.3` 显示成 `9.3`，`10.0` 显示成 `10`） |

**为什么需要 float**：资源可以有非整数速率（信仰每刻 −0.7、磐核每刻 +0.4）。
`apply` 的叠层分支读 `float()`（`effects.py:418`）、`consume` 读 `float()`
（`effects.py:523`）、`schedule` 的 gain 分支 `round(..., 6)`（`schedule.py:660`）。
读侧全用 `float()` 保真，写侧统一过 `norm_stack` 保持「int 资源看起来还是 int」。

## cap 的唯一收敛点

```python
def _cap_of(actor, key):                       # effects.py:59，S2 公开别名 cap_of
...
读它的地方（都是叠层 clamp）：`act_apply` 叠层分支（`effects.py:419`）、
`schedule` 周期 gain 分支（`schedule.py:657`，且**表声明可被 `period.cap` 覆盖**）。
...
引擎的实现要点（`schedule._settle_time_effects`，`schedule.py:466-610`）：

1. **首次挂不给跳**：第一次看到某 key 时只登记 `dot_next[key] = now + interval`
   （`schedule.py:494-497`，对齐旧引擎的「首跳延迟」）
...
4. **`turns` 限跳**：跳够 `turns` 次就清层（计数器 `dot_jumps`，`schedule.py:666-672`）
5. **`dir="gain"` 不需要 `stacks > 0`**：0 层也要回（游侠精力耗到 0 若被拦将永远回不了，
   `schedule.py:487-490` 注释）

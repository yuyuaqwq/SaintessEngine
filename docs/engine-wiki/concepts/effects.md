...
- 到期只有一处（`schedule._settle_time_effects`，`schedule.py:437-456`）
- 净化只有一处（`effects.act_cleanse`，`effects.py:628-637`）
...
例 `schedule.py:408`、`stats.py:54`、`effects.py:485`）。
...
| `period` | **内容侧**直接写入 | `schedule._settle_time_effects`（`schedule.py:479-487`） | 动态周期声明（条目自带优先，回落表声明） |
| `value` | 内容侧（`heal_amp_pct` 等） | `landing._apply_heal_mods`（`landing.py:518`） | 附加数值袋（形态自定，消费方自己解释） |

### `v` 字段的消费缺口

`act_apply` 的值型分支会写 `{"v": 0.45}`（`effects.py:475-477`），并给 `reduce` 额外写
`holder["reduce_left"]`（`effects.py:478-479`）。但：
...
| 函数 | 行为 |
|---|---|
| `_norm_stack(v)` / `norm_stack`（`effects.py:50`，S2 公开别名 `:80`） | 写回归一：整值 → `int`；小数 → `round(6)`（清 `10-0.7 → 9.3` 的二进制尾差） |
...
```python
def _cap_of(actor, key):                       # effects.py:62，S2 公开别名 cap_of
...
引擎的实现要点（`schedule._settle_time_effects`，`schedule.py:473-622`）：

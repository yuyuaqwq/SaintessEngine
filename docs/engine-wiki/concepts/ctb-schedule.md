...
`schedule.py:152`），它内部依次做「加时钟 → 结算周期/到期 → 广播 `time_advance`」。
所以「3 秒内发生了 3 次 DOT」这件事是确定的、可断言的，不依赖主循环被调用了几次。

## 公式

```python
def action_time(spd, base=CAST_ATK):          # schedule.py:28
...
| `SPD_REF` | 50.0 | `schedule.py:24` |

（`CAST_ITEM` / `HOT_INTERVAL` / `DEFAULT_CT_WAIT` 零消费，**已删**，2026-09-11；
道具耗时由命令层自己算。）
...
不是裸 `actor["spd"]`。播种（`battle._seed_ct_one`，`battle.py:90`）、
行动后推进（`schedule._after_act`，`schedule.py:139`）、`next_ct`（`schedule.py:42`）
...
```python
initial_ct(spd)        # schedule.py:37 = action_time(spd) —— 开局第一动的等待
action_time(spd, base) # schedule.py:28 —— 单次行动耗时
next_ct(battle, actor) # schedule.py:42 —— 返回 actor 行动后的 ct（绝对时刻）
```
...
```python
def advance(battle, logs, max_steps=200):     # schedule.py:67
...
> ⚠️ **`human_act` 不检查 ct**：它拿到 caster 就直接 `act()`（`battle.py:234-254`），
...
`_advance_time(battle, dt, logs)`（`schedule.py:152`）：
...
`now` 已经是结算后的状态（`schedule.py:155-157` 注释：挂敌身条等按刻连续结算的
声明订阅此事件，「读点永远拿到当刻值」）。

`_settle_time_effects`（`schedule.py:171`）三轮：
...
返回**数字**则当作绝对秒直接落 `ct`（`battle.py:262-271`）。

## 控制效果如何与时间轴互动

被控（`mode="skip"`）时的语义是「**行动浪费**」（`battle.py:429-436`）：
...
「照样推 ct」由调用方完成：`actor_auto`（`battle.py:383-385`）或 `human_act`
（`battle.py:259-273`）在 `act()` 返回后都调 `_after_act`。所以控制不是「冻结时间」，
而是「这次行动白费」——这是 CTB 类游戏的标准语义。

`mode="no_skill"`（沉默）不跳行动，只把 `attack` 换成 `skill` 清掉技能名
（`battle.py:424-427`），耗时按 `attack` 计（`human_act` 里 `ctx.action` 已被改写，
`battle.py:256-257` 注释）。

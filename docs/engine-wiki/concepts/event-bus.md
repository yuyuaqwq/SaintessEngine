...
- **有些事件故意不带 `actor`**：`act_done` 只放 `ctx["acted"]`，让效果侧自己判敌我
  （`battle.py:588-590` 注释：`randuin`/`ice_vein` 靠它监听「敌对 actor 行动」叠减速）。
...
```
（`actions.py:438-445`，同款出现在 `landing.py:83-94` 的 `taken_calc`、
`actions.py:677-747` 的 `heal_calc`、`schedule.py:599-615` 的 `dot_calc`）
...
※ `skill_hit` / `attack_hit` 的 fire 点位用变量选事件名（`actions.py:488`：

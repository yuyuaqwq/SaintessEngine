...
> ⚠️ **未知名被静默忽略**：`set_hook`（`config.py:123`）里 `if name in _HOOKS`
...
config.load_game_rules(my_rules_module)               # config.py:88
```
...
| 函数 | 位置 | 语义 |
|---|---|---|
| `get_effect_actions()` | `config.py:100` | 默认 `{}` |
| `get_effect_rules()` | `config.py:105` | 默认 `{}` |
| `state_def(key)` | `config.py:110`（`state_effects.py:13` 的实体） | `get_effect_rules().get(key) or {}` |
...
| **`config.strict = True`** | `get_hook` 对未装配 hook 抛 `EngineNotConfigured`（`config.py:146`），错误信息直接点名缺哪个 hook |
...
```python
config.register_hook_provider(my_lazy_mount)   # config.py:94
```

引擎首次访问某个未装配 hook 时调用一次（`_lazy_bootstrap`，`config.py:152`，带防重入）。

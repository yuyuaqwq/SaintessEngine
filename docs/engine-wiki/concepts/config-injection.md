...
| `kinds` | dict | `config.kind_of`（`config.py:252`）→ `actions._kind` | `""`（kind 比较全不成立） |
...
> ⚠️ **未知名被静默忽略**：`set_hook`（`config.py:140`）里 `if name in _HOOKS`
...
config.load_game_rules(my_rules_module)               # config.py:105
```
...
| 函数 | 位置 | 语义 |
|---|---|---|
| `get_effect_actions()` | `config.py:117` | 默认 `{}` |
| `get_effect_rules()` | `config.py:122` | 默认 `{}` |
| `state_def(key)` | `config.py:127`（`state_effects.py:13` 的实体） | `get_effect_rules().get(key) or {}` |
...
| **`config.strict = True`** | `get_hook` 对未装配 hook 抛 `EngineNotConfigured`（`config.py:163`），错误信息直接点名缺哪个 hook |

```python
config.strict = True    # 开发/测试环境建议打开（config.py:88）
```

原文说明（`config.py:83-87`）：`False`（默认）与历史行为一致——未装配给中性兜底不炸；
...
```python
config.register_hook_provider(my_lazy_mount)   # config.py:111
```

引擎首次访问某个未装配 hook 时调用一次（`_lazy_bootstrap`，`config.py:169`，带防重入）。

...
`_HOOKS`（`config.py:30-96`）白名单，`mount(**hooks)` / `set_hook(name, value)` 写，
`get_hook(name)` 读。

| hook | 类型 | 引擎在哪里用 | 不装配的行为 |
|---|---|---|---|
| `formulas` | 对象 | `actions._skill_seg_damage`、`_settle_lifesteal`、`_do_heal`、`skill_pay_of` 等（`actions.py:121,355,492,500,602,630,739,769`） | 退回 `_NullFormulas`（全零效应，**静默**） |
...
| `skill_lookup` | 对象（需 `.skill_info(cls, key)` / `.skill_by_key(key)`） | `battle._index_one_actor`（`battle.py:151,137`） | 返回 `None` → 技能索引空 |
| `monster_skill_fn` | `fn(key) -> dict\|None` | `battle._index_one_actor`（`battle.py:157`） | `None` |
| `basic_skill_fn` | `fn(class_name) -> dict\|None` | `actions.resolve_basic_skill`（`actions.py:39`） | 回落 `basic_fallback` |
| `basic_fallback` | dict | 同上（`actions.py:47`） | 结构化兜底 `{"name": "", "kind": "", "exprs": ["atk*1.0"]}` |
...
`expr_vars_fn` 是**表达式变量**的表读点：引擎只认识「变量名 / 取值来源（stat · input · const）/
显示名」这个形状，「这款游戏有哪些变量」由内容侧声明（默认表 = 引擎历史那一份）。
...
> ⚠️ **未知名被静默忽略**：`set_hook`（`config.py:139`）里 `if name in _HOOKS`
...
config.load_game_rules(my_rules_module)               # config.py:120
```
...
| 函数 | 位置 | 语义 |
|---|---|---|
| `get_effect_actions()` | `config.py:135` | 默认 `{}` |
| `get_effect_rules()` | `config.py:149` | 默认 `{}` |
...
| **`config.strict = True`** | `get_hook` 对未装配 hook 抛 `EngineNotConfigured`（`config.py:169`），错误信息直接点名缺哪个 hook |

```python
config.strict = True    # 开发/测试环境建议打开（config.py:100）
```
...
```python
config.register_hook_provider(my_lazy_mount)   # config.py:132
```

引擎首次访问某个未装配 hook 时调用一次（`_lazy_bootstrap`，`config.py:175`，带防重入）。

...
| `kinds` | dict | `config.kind_of`（`config.py:259`）→ `actions._kind` | `""`（kind 比较全不成立） |
...
> ⚠️ **未知名被静默忽略**：`set_hook`（`config.py:147`）里 `if name in _HOOKS`
...
| 函数 | 位置 | 语义 |
|---|---|---|
| `get_effect_actions()` | `config.py:124` | 默认 `{}` |
| `get_effect_rules()` | `config.py:129` | 默认 `{}` |
| `state_def(key)` | `config.py:134`（`state_effects.py:13` 的实体） | `get_effect_rules().get(key) or {}` |
...
| **`config.strict = True`** | `get_hook` 对未装配 hook 抛 `EngineNotConfigured`（`config.py:170`），错误信息直接点名缺哪个 hook |
...
```python
config.register_hook_provider(my_lazy_mount)   # config.py:118
```

引擎首次访问某个未装配 hook 时调用一次（`_lazy_bootstrap`，`config.py:176`，带防重入）。

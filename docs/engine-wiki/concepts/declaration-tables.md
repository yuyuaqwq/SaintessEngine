...
这是理解声明系统最关键的一节。`act_apply`（`effects.py:298`）按**参数**分流：
...
（`effects.py:319-432`，逐支的注释与日志文案都在那一段）

`op="add"` 与 `op="mul"` 的区别是**有没有 `stat`**：面板增益的 `op` 是面板算子且必带
`stat`，所以走快照分支（`effects.py:373-376` 注释）。
...
| `panel` | `effects.act_apply` 快照分支（`effects.py:431-443`） |
| `consume.mode` | `effects.act_apply` 控制分支（`effects.py:322-325`）+ `Battle.act` 的控制消费（`battle.py:435-458`） |
| `period` | `schedule._settle_time_effects`（`schedule.py:235-243`） |
| `cleanse` / `period` / `on=="target"` | `effects.act_cleanse`（`effects.py:552-556`） |

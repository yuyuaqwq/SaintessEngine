...
| 表 | 回答的问题 | 消费者（引擎侧） |
|---|---|---|
| `EFFECT_ACTIONS` | 这个**名词**对应哪些**动词**？ | `effects.resolve_actions`（`effects.py:137`） |
...
- **可审计**：一张表能 diff、能生成文档、能被脚本校验（**游戏仓侧**确有这类脚本，
...
这是理解声明系统最关键的一节。`act_apply`（`effects.py:337`）按**参数**分流：
...
（`effects.py:353-482`，逐支的注释与日志文案都在那一段）

`op="add"` 与 `op="mul"` 的区别是**有没有 `stat`**：面板增益的 `op` 是面板算子且必带
`stat`，所以走快照分支（`effects.py:423-426` 注释）。
...
| 字段 | 谁读它 |
|---|---|
| `cap` | `effects._cap_of`（`effects.py:62`）← 叠层 clamp 的**唯一收敛点** |
...
| `panel` | `effects.act_apply` 快照分支（`effects.py:468-480`） |
| `consume.mode` | `effects.act_apply` 控制分支（`effects.py:356-359`）+ `Battle.act` 的控制消费（`battle.py:481-508`） |
| `period` | `schedule._settle_time_effects`（`schedule.py:479-487`） |
| `cleanse` / `period` / `on=="target"` | `effects.act_cleanse`（`effects.py:628-637`） |
...
| 路径 | 谁翻译 | 什么时候 |
|---|---|---|
| 技能 `effect` 字段 / `mech` 字段 | `apply_effects`（`effects.py:168`） | 命中后 / 施放时 |
...
`EFFECT_ACTIONS`（`effects.py:201-205`：`act_name = act.get("action") or etype`，

...
| 表 | 回答的问题 | 消费者（引擎侧） |
|---|---|---|
| `EFFECT_ACTIONS` | 这个**名词**对应哪些**动词**？ | `effects.resolve_actions`（`effects.py:135`） |
...
这是理解声明系统最关键的一节。`act_apply`（`effects.py:326`）按**参数**分流：
...
（`effects.py:347-460`，逐支的注释与日志文案都在那一段）

`op="add"` 与 `op="mul"` 的区别是**有没有 `stat`**：面板增益的 `op` 是面板算子且必带
`stat`，所以走快照分支（`effects.py:401-404` 注释）。
...
| `period` | `schedule._settle_time_effects`（`schedule.py:268-276`） |
| `cleanse` / `period` / `on=="target"` | `effects.act_cleanse`（`effects.py:583-592`） |
...
| 路径 | 谁翻译 | 什么时候 |
|---|---|---|
| 技能 `effect` 字段 / `mech` 字段 | `apply_effects`（`effects.py:165`） | 命中后 / 施放时 |
...
`EFFECT_ACTIONS`（`effects.py:197-201`：`act_name = act.get("action") or etype`，

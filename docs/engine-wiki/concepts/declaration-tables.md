...
| 表 | 回答的问题 | 消费者（引擎侧） |
|---|---|---|
| `EFFECT_ACTIONS` | 这个**名词**对应哪些**动词**？ | `effects.resolve_actions`（`effects.py:140`） |
...
| `consume.mode` | `effects.act_apply` 控制分支（`effects.py:354-357`）+ `Battle.act` 的控制消费（`battle.py:482-509`） |
| `period` | `schedule._settle_time_effects`（`schedule.py:622-630`） |
...
| 路径 | 谁翻译 | 什么时候 |
|---|---|---|
| 技能 `effect` 字段 / `mech` 字段 | `apply_effects`（`effects.py:171`） | 命中后 / 施放时 |
...
`EFFECT_ACTIONS`（`effects.py:204-208`：`act_name = act.get("action") or etype`，

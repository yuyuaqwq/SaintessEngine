...
- `add_actor()` 加援军/召唤物时不需要给调度器注册新类型（`battle.py:252`）
- 序列化不需要按类型分派（`serialize._serialize_actor` 对任何 actor 一视同仁，`serialize.py:51`）

代价：**身份信息全靠字段**。要表达「这是 Boss」就写 `is_boss=True` 或 `role="boss"`
（引擎真读这两个的地方：控制时长减半 `effects.py:385`、DOT 的 `pct_boss` / `boss_pct_mult` 档 `schedule.py:515`、
...
（例：`schedule._after_act` 用 `stats.actor_spd`，`schedule.py:362`）。
...
| `defending` | bool | 防御姿态（`deal_damage` 里减伤 50%，`landing.py:174`） |
...
| `act_count` | int | 个体行动计数，`actor_auto` 每动 +1（`battle.py:439`） |
| `dot_next` / `dot_jumps` | `{key: 数值}` | 周期结算的运行期辅助（`schedule.py:468-469` 惰性建） |
...
| `auto_act` | 自动行动配置（`actor_auto` 读它，`battle.py:388`） |
...
| `bonus.cap` | `effects._cap_of`（`effects.py:75`） | `{资源key: 上限增量}`，纯 flat int 加在 `EFFECT_RULES[key].cap` 上 |
| `bonus.cost` | `actions._bonus_cost_of`（`actions.py:243`） | 技能消耗折扣（`mp_pct`/`mp_flat`/`res` + `when` 判据） |

...
（引擎真读这两个的地方：控制时长减半 `effects.py:382`、DOT 的 `pct_boss` / `boss_pct_mult` 档 `schedule.py:513`、
...
（例：`schedule._after_act` 用 `stats.actor_spd`，`schedule.py:361`）。
...
| `dot_next` / `dot_jumps` | `{key: 数值}` | 周期结算的运行期辅助（`schedule.py:467-468` 惰性建） |
...
| `bonus.cap` | `effects._cap_of`（`effects.py:74`） | `{资源key: 上限增量}`，纯 flat int 加在 `EFFECT_RULES[key].cap` 上 |

...
| `on_threshold` | 全仓 grep → 只有 `effects.py:249/244/250` 的判据关键词 + `battle_rules.py:27` 的声明 | **无消费方**。`threshold` **事件**有引擎点位（`effects.py:461`），但这张映射表没被读 |
...
| `tag` | grep `state_def(...).get("tag")` / `cfg.get("tag")` → 空 | **无消费方**。`act_apply` 读的是 params 的 `tag`（作 key 兜底，`effects.py:349`） |
| `dot` | 全仓 grep → 只有 `effects.py:285` 判据；78 个 key 里无一使用 | **无消费方**（V5 后 DOT 统一走 `period`） |
...
| `dmg_type` | **2026-09-11 已接线**：DOT 落地改传 `dmg_kind=period.get("dmg_type")`（`schedule.py:619`） | ✅ **已消费**。`corros` 的「真伤 DOT」声明现成立（真伤 → 物免/魔免/格挡全跳过）；非真伤 DOT 仍空 kind，行为与接线前一致 |
...
| `actions._aoe_falloff_apply` | `actions.py:556` | **占位实现**（原样返回 logs）。`info["aoe_falloff"]` 在 `actions.py:367` 被读取但随后被丢弃 → AOE falloff 实际未生效 |
...
| `effects.effects_from_skill(..., caster_side_is_player=True)` | `effects.py:222` | **第三个参数在函数体里从未使用** |
...
| `actor["dot_next"]` / `actor["dot_jumps"]`（dict） | 引擎周期结算的运行期辅助（`schedule.py:469-470` 惰性建），**同样落盘**。这是「续战能对上」的原因，但字段名与内容无关 |
| `actor["_dmg_taken_mult"]`（float） | 承伤乘区（`landing.py:96-103` 读）。由上层直写（例 游戏仓 `commands/boss_script.py:684`）；**同样落盘** |
| `actor["reduce_left"]` / `reduce_all_left` | `effects.act_apply` 写（`effects.py:482`）+ 内容侧 bridge 透传/播种；**无消费者**（见 §1.3） |
...
| `skill_hit` / `attack_hit` | **有点位但静态 grep 不到**：`actions.py:488` 用变量选事件名（`ev = "attack_hit" if info.get("_basic") else "skill_hit"`）。文档若按 grep 结果断言「无点位」会是错的 |
...
| B3 | `effects.act_apply` 里 `if key == "reduce":` | `effects.py:481` |
| B4 | `is_boss` / `role == "boss"`（控制减半 / DOT `pct_boss`） | `effects.py:388` · `schedule.py:516,286` |
| B5 | `battle.py` 里 `"player"` 阵营名 | `battle.py:190, 515` |
| B6 | `_is_stack_resource` 的判据关键词含无消费方的字段（`debuff_scale` / `dot` / `on_threshold` / `guard_hp_pct`） | `effects.py:282-286` |

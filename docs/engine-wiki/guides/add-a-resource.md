...
| 字段 | 作用 | 引擎侧消费者 |
|---|---|---|
| `cap: 5` | 叠层上限 | `effects._cap_of`（`effects.py:64`）；`apply op=add` 与 `schedule` gain 都 clamp |
...
- 扣费：`_spend_skill_cost`（`actions.py:203`）——`stacks = norm_stack(max(0, cur - rv))`

⚠️ **一个重要的历史行为**：`res_cost` 只在 `actor.effects` **已经有该 key 条目**时才拦截
（`actions.py:192-195` 的 `continue`）。没条目 = 不拦（保持历史行为）。
...
- 引擎侧消费者：`schedule._settle_time_effects` 的 `gain` 分支（`schedule.py:768-787`）
- **静默**（不刷日志，`schedule.py:775-776` 注释）
- clamp 到 `[0, cap]`，cap 取 `period.cap` 或 `_cap_of`（表声明 + `bonus.cap`）
- 负数也走（信仰清醒档衰减），但**下限 0**，不会归负
- `dir="gain"` **不要求 `stacks > 0`** —— 0 层也要能回（`schedule.py:634-637`）

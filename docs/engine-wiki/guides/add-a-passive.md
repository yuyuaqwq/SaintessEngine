...
| `cost` | `actor["bonus"]["cost"]`（`mp_pct` / `when[].mp_pct`） | `actions._skill_pay_of`（`actions.py:279`） | `passive.mp_mult`（如 0.5 = 打五折） |
...
| 我想... | 用 | 为什么 |
|---|---|---|
| 影响**我打出的伤害**（增伤/处决/破魔） | `dmg_calc` | 攻击方视角乘区，`actions.py:451` |
| 影响**我受到的伤害**（减伤/护盾转化） | `taken_calc` | 承伤方视角乘区，`landing.py:83` |
| 影响**我造成的治疗**（治疗增幅） | `heal_calc` | 施法者视角乘区，`actions.py:690` |

...
（`ctx["acted"]` 才是行动者，`battle.py:477-482`），所以「敌方行动后我叠减速」
...
读取与折算在 `_skill_pay_of`（`actions.py:266-301`），两条硬规则（`actions.py:228-234` 注释）：
...
`when` 的判据由 `_cost_judge_hit`（`actions.py:246`）实现，支持三个谓词：

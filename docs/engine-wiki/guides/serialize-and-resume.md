...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:680/532`）。
...
| `act_count` | `actor_auto` 每动 +1（`battle.py:442`） | AI `round_mod` 谓词读它 |
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:616`），
...
`killed_actors`，`battle.py:633-651`；`_check_side_end` 也不删）。

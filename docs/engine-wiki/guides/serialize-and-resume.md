...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:677/532`）。
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:613`），
...
`killed_actors`，`battle.py:630-648`；`_check_side_end` 也不删）。

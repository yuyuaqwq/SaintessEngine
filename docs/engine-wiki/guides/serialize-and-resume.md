...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:684/532`）。
...
| `dot_next` / `dot_jumps` | 引擎周期结算辅助（`schedule.py:469-470`） | 落盘是**续战能对上**的原因，别手删 |
...
| `reduce_left` | `effects.act_apply`（`effects.py:482`） | ⚠️ 无消费者 |
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:620`），
...
`killed_actors`，`battle.py:637-655`；`_check_side_end` 也不删）。

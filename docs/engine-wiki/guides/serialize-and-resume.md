...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:556/532`）。
**两条路等价**，内容层两种都在用（游戏仓侧拆仓前的迁移计划 `docs/ENGINE_CONTENT_SPLIT_PLAN.md` §5 记了
...
| `act_count` | `actor_auto` 每动 +1（`battle.py:381`） | AI `round_mod` 谓词读它 |
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:496`），
...
`killed_actors`，`battle.py:511-527`；`_check_side_end` 也不删）。

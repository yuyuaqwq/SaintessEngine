...
`Battle.to_state` / `Battle.from_state` 是类方法包装（`battle.py:657/532`）。
...
| `dot_next` / `dot_jumps` | 引擎周期结算辅助（`schedule.py:464-465`） | 落盘是**续战能对上**的原因，别手删 |
...
`battle_start` 事件是**整场一次**的（`_ensure_battle_started`，`battle.py:597`），
...
`killed_actors`，`battle.py:612-628`；`_check_side_end` 也不删）。

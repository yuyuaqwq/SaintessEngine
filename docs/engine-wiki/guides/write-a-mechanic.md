...
1. **落地一定走 `landing`**
   ...
   - 治疗：`landing.heal_actor(battle, target, amount, logs, source=None, label="")`（`landing.py:435`）
...
| `dot_calc` | DOT 每跳 | ×mult DOT 增伤 | `schedule.py:597-613` |
...
| 一次性行动 | `Battle.action_override`（`battle.py:521`） | `use_item` 类自定义行动 |

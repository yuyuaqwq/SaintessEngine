...
```
命令层
  └─ Battle.human_act(action, skill_name, actor, target)        battle.py:275
       ├─ ActCtx(caster, action, skill_name, target, ...)       actors.py:20
       ├─ Battle.act(ctx)                                       battle.py:446
       │    ├─ _ensure_battle_started()                         battle.py:613   ⚡ battle_start
       │    ├─ fire("turn_start")                               battle.py:466   ⚡
       │    ├─ 控制消费（mode=skip → 早退；no_skill → 技能转普攻） battle.py:478-505 ⚡ on_act_consume
       │    ├─ fire("act_begin")                                battle.py:507   ⚡
       │    ├─ _p_acts += 1                                     battle.py:465
       │    ├─ 分派：
       │    │    attack → actions.do_attack                      actions.py:50
       │    │    skill  → actions.do_skill                       actions.py:63
       │    │    defend → Battle._do_defend                      battle.py:597
       │    │    flee   → Battle._do_flee                        battle.py:596
       │    │    其他   → Battle.action_override 注入点           battle.py:518
       │    ├─ fire("act_done")                                 battle.py:586   ⚡
       │    └─ _check_side_end()                                battle.py:650
       └─ 若未结束且 action ∈ {attack, skill, defend} 或 override 被消费：
            ├─ schedule._after_act(battle, caster, action)      schedule.py:361  推 ct
            └─ Battle.advance(logs)                             battle.py:332
                 └─ schedule.advance（推时钟 + 自动 actor 行动）  schedule.py:284
```

## 展开 1：`advance` 的循环

```
schedule.advance(battle, logs, max_steps=200)                     schedule.py:284
  while battle.result is None and guard < 200:
    ...
    if fp 先到点（fp[1] <= auto[1] + 1e-9）:
        ...
        logs.append("—— {name} 行动 ——")
        battle.actor_auto(actor)                                       battle.py:357
```

`_advance_time(battle, dt, logs)`（`schedule.py:375`）内部：

```
battle._now += dt
_settle_time_effects(battle, logs)                                  schedule.py:284
   ├─ for 每个存活 actor:
   │    ├─ effects 到期 → pop + ⚡ buff_expire                        schedule.py:407
   │    ├─ shields 到期（expire_at <= now）→ pop                      schedule.py:457-467
   │    └─ 周期跳（period）:
   │         首次 → dot_next[key] = now + interval（不跳）             schedule.py:500-503
...
fire("time_advance", {"dt": dt, "now": battle._now})                schedule.py:407 ⚡
```
...
| `act_done` 不带 `actor` 键 | 所以它是全员广播；行动者放在 `ctx["acted"]` | `battle.py:581-586` |
| 被控跳过时 `act()` **提前 return**，`act_done` 不 fire | 推 ct 由调用方做 | `battle.py:496-505` |

...
- **函数内 import 制造了恰好 2 对双向互指**：`effects ↔ effect_triggers`
  （`effects.py:444` / `effect_triggers.py:100`）与 `battle ↔ serialize`
  （`battle.py:679` / `serialize.py:64`）。这两对都是「延迟 import 破环」的写法，
...
1. **落地只能走 `landing`**。任何模块自己扣 `hp` 都会绕过护盾/死亡/事件。
   （例：`schedule` 的 DOT 结算也调 `landing.deal_damage`，`schedule.py:605`）

...
| R5 | `actions.py:716` → `game.core.formula_expr` | 合规（通用解释器） |
| R6 | `actions.py:763` → `game.engine.skill_buff_turns` | 反向边 |
| R7 | `actions.py:766` → `game.core.constants` | 死 import |
| R8 | `actions.py:795` → `game.engine.skill_mech_val` | 反向边 |
| R9 | `battle.py:110` → `game.engine`（技能表查询） | 反向边 |
| R10 | `battle.py:111` → `game.content.MONSTER_SKILLS` | 内容表直读 |
...
| B7 | `battle.py` 里 `"player"` 阵营名硬编码 | `battle.py:545`（`_check_side_end`）、`:170`（`focus`） | 你的游戏若不叫 `player` 就得改引擎或用 `hostile_map` 绕过 |
...
`actions._do_heal` 里 `cond_mult = 1.0  # N2b 补，恒 1.0 起步`（`actions.py:675`）。

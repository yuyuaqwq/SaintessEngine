...
| 都没有 | `[]` → 调用方 `continue`（**静默 no-op**，`effects.py:192-193`） |
...
| 动词 | 注册行 | 参数要点 |
|---|---|---|
| `apply` | `effects.py:327` | 五形态按参数分流：`mode`→控制 / `op=add\|set`+无 `stat`→叠层 / `value`\|`pct_from_mech_val`→值型 / `stat`+`mult`→面板快照 / `hit`→出手消费 / 无→纯状态 |
| `consume` | `effects.py:511` | `key` + `amount`（不足则**不扣**并写提示，`effects.py:521-523`） |
| `shield` | `effects.py:544` | `key`（缺省 `"buff"`）/ `value` \| `pct` \| 缺省 20% max_hp / `turns`（缺省 3；`>=999` 或 `forever` = 永久）/ `halve` |
| `cleanse` | `effects.py:595` | 遍历目标 `effects`，按 `EFFECT_RULES[key]` 的 `period`（**`dir="gain"` 的资源回除外**）\| `on=="target"` \| `cleanse` 三判据清 |
...
`mode` 走 `EFFECT_RULES[key].consume.mode` 查表（`effects.py:351-354`）。
...
> | 漏了什么 | 引擎行为 | 代码 |
> |---|---|---|
> | `key`（或 `tag` / `mech`）全缺 | `if not key: return` → 效果**完全不施加** | `effects.py:315-308` |
> | `turns` ≤ 0 / 不写 | 控制分支 `if turns <= 0: return` → **不施加**（`turns` 不会被默认值兜底） | `effects.py:302-304` |
> | `on` 不写 | 缺省值是 **`"caster"`**（`params.get("on", "caster")`）→ **把控制挂到自己身上**（晕自己） | `effects.py:344, 300` |
>
> 正确写法 = 三个都给：
>
> ```python
...
`effects.py:345`）；`heal_self` 没给 `pct`（靠 `info.hp_pct`，`effects.py:658`）；
`shield` 没给 `value`（缺省取 `max_hp × 20%`，`effects.py:565`）。
...
`_is_stack_resource` 的判据（`effects.py:241-256`）：
...
`cc_turns` 的注释值得逐字读（`effects.py:254-258`）：

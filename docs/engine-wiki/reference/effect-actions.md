...
| 都没有 | `[]` → 调用方 `continue`（**静默 no-op**，`effects.py:196-197`） |
...
| 动词 | 注册行 | 参数要点 |
|---|---|---|
| `apply` | `effects.py:336` | 五形态按参数分流：`mode`→控制 / `op=add\|set`+无 `stat`→叠层 / `value`\|`pct_from_mech_val`→值型 / `stat`+`mult`→面板快照 / `hit`→出手消费 / 无→纯状态 |
| `consume` | `effects.py:524` | `key` + `amount`（不足则**不扣**并写提示，`effects.py:521-523`） |
| `shield` | `effects.py:557` | `key`（缺省 `"buff"`）/ `value` \| `pct` \| 缺省 20% max_hp / `turns`（缺省 3；`>=999` 或 `forever` = 永久）/ `halve` |
| `cleanse` | `effects.py:608` | 遍历目标 `effects`，按 `EFFECT_RULES[key]` 的 `period`（**`dir="gain"` 的资源回除外**）\| `on=="target"` \| `cleanse` 三判据清 |
| `cleanse_all` | `effects.py:642` | 同上，`target or caster` |
| `heal` | `effects.py:650` | `pct`（max_hp 比例）/ `missing_pct`（已损比例）/ `value`；`info.hp_pct` 兜底 |
| `interrupt` | `effects.py:689` | 清 `target["charging"]`，fire `interrupt` |
| `damage` | `effects.py:718` | `value` / `pct`（`pct_max_hp` 别名）/ `kind`；`on=target`（缺省）或 `on=caster`（自伤/反伤） |
...
`mode` 走 `EFFECT_RULES[key].consume.mode` 查表（`effects.py:355-358`）。
...
> | `on` 不写 | 缺省值是 **`"caster"`**（`params.get("on", "caster")`）→ **把控制挂到自己身上**（晕自己） | `effects.py:353, 300` |
>
> 正确写法 = 三个都给：
>
> ```python
...
`effects.py:345`）；`heal_self` 没给 `pct`（靠 `info.hp_pct`，`effects.py:671`）；
`shield` 没给 `value`（缺省取 `max_hp × 20%`，`effects.py:578`）。
...
`_is_stack_resource` 的判据（`effects.py:243-258`）：
...
`cc_turns` 的注释值得逐字读（`effects.py:259-263`）：

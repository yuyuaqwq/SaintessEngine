...
|---|---|---|---|---|---|
| 1 | `battle_start` | `battle.py:628`（`_ensure_battle_started`） | `{}` | **无**（全体触发） | 首个 actor 行动前，**整场一次**（`_started` 守卫） |
| 2 | `turn_start` | `battle.py:469`（`act`） | `actor` | 行动者 | 回合开始，**先于控制检查**（所以「回合开始回蓝」被晕也触发） |
| 3 | `act_begin` | `battle.py:510`（`act`） | `actor`, `target` | 行动者 | 控制通过、行动执行前 |
| 4 | `act_cast` | `actions.py:112`（`do_skill`） | `actor`, `target`, `info` | 施法者 | 扣费与冷却之后、结算之前 |
| 5 | `skill_hit` | `actions.py:435-437`（`_single_target_pipeline`） | `actor`, `target`, `info`, `dmg` | 攻击者 | 技能命中后（伤害已落地）；AOE 每目标各触发一次 |
...
| 11 | `on_death` | `battle.py:648`（`_on_actor_dead`） | `actor`, `target` | 死者 | 所有死亡路径统一在此；**死者的声明仍会执行**（subject 例外） |
| 12 | `dot_tick` | `schedule.py:628`（`_settle_time_effects`） | `actor`, `target`, `key`, `dmg` | 受跳者 | DOT 每一跳 |
| 13 | `dot_calc` | `schedule.py:597`（同上） | `target`, `dot_key`, `dmg`, `mult` | **无**（广播） | DOT 伤害落地**前**的乘区钩子 |
| 14 | `on_act_consume` | `battle.py:498`（`act`） | `actor`, `tag` | 行动者 | 被控跳过行动（`mode="skip"`）时 |
| 15 | `on_hit_consume` | `actions.py:537`（`_consume_hit_buffs`） | `actor`, `key` | 出手者 | 一次性出手 buff 被消费时 |
| 16 | `buff_expire` | `schedule.py:578`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
| 17 | `threshold` | `effects.py:458`（`act_apply` 叠层分支） | `actor`, `key`, `value` | 条目持有者 | 叠层数值变化后（「战意满 10 → 狂暴」类） |
| 18 | `dmg_calc` | `actions.py:440`（`_single_target_pipeline`） | `actor`, `target`, `dmg`, `is_crit`, `info`, `mult` | 攻击者 | 伤害算出后、落地前（攻击方乘区） |
| 19 | `taken_calc` | `landing.py:119`（`deal_damage`） | `actor`, `target`, `source`, `dmg`, `mult` | 承伤者 | 承伤修正（承伤方乘区） |
| 20 | `heal_calc` | `actions.py:736`（`_do_heal`） | `actor`, `target`, `heal`, `info`, `mult` | 施法者 | 治疗量算出后、落地前 |
| 21 | `act_done` | `battle.py:589`（`act` 尾部） | `acted`（**不是** `actor`） | **无**（全员广播） | 行动完成；被控跳过不触发 |
| 22 | `phase` | ⚠️ **无引擎点位** | — | — | Boss 阶段转换（上层驱动） |
| 23 | `player_low` | ⚠️ **无引擎点位** | — | — | 玩家低血量（上层驱动） |
| 24 | `pv_broken` | ⚠️ **无引擎点位** | — | — | 破防（上层驱动） |
| 25 | `interrupt` | `landing.py:208`（伤害打断蓄力）/ `effects.py:700`（`act_interrupt` 动词） | `actor`, `target`, `source` | 被打断者 | 读条被打断 |
| 26 | `time_advance` | `schedule.py:408`（`_advance_time`） | `dt`, `now` | **无**（广播） | 时钟推进（结算**之后**广播） |
...
（`actions.py:345-429` + `landing.py:28-162` + `battle.py:449-545`）

...
|---|---|---|---|---|---|
| 1 | `battle_start` | `battle.py:625`（`_ensure_battle_started`） | `{}` | **无**（全体触发） | 首个 actor 行动前，**整场一次**（`_started` 守卫） |
| 2 | `turn_start` | `battle.py:466`（`act`） | `actor` | 行动者 | 回合开始，**先于控制检查**（所以「回合开始回蓝」被晕也触发） |
| 3 | `act_begin` | `battle.py:507`（`act`） | `actor`, `target` | 行动者 | 控制通过、行动执行前 |
...
| 12 | `dot_tick` | `schedule.py:616`（`_settle_time_effects`） | `actor`, `target`, `key`, `dmg` | 受跳者 | DOT 每一跳 |
| 13 | `dot_calc` | `schedule.py:585`（同上） | `target`, `dot_key`, `dmg`, `mult` | **无**（广播） | DOT 伤害落地**前**的乘区钩子 |
| 14 | `on_act_consume` | `battle.py:495`（`act`） | `actor`, `tag` | 行动者 | 被控跳过行动（`mode="skip"`）时 |
| 15 | `on_hit_consume` | `actions.py:537`（`_consume_hit_buffs`） | `actor`, `key` | 出手者 | 一次性出手 buff 被消费时 |
| 16 | `buff_expire` | `schedule.py:566`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
...
| 21 | `act_done` | `battle.py:586`（`act` 尾部） | `acted`（**不是** `actor`） | **无**（全员广播） | 行动完成；被控跳过不触发 |
...
（`actions.py:345-429` + `landing.py:28-162` + `battle.py:446-542`）

...
|---|---|---|---|---|---|
| 1 | `battle_start` | `battle.py:607`（`_ensure_battle_started`） | `{}` | **无**（全体触发） | 首个 actor 行动前，**整场一次**（`_started` 守卫） |
| 2 | `turn_start` | `battle.py:459`（`act`） | `actor` | 行动者 | 回合开始，**先于控制检查**（所以「回合开始回蓝」被晕也触发） |
| 3 | `act_begin` | `battle.py:493`（`act`） | `actor`, `target` | 行动者 | 控制通过、行动执行前 |
...
| 8 | `on_taken` | `landing.py:203`（`deal_damage`） | `actor`, `target`, `source`, `dmg` | 受击者 | 承伤落地后；**死者不触发**（走 `on_death`） |
| 9 | `on_heal` | `landing.py:460`（`heal_actor`） | `actor`, `target`, `source`, `amount`, `overflow` | 被治疗者 | 实际回血 > 0 时 |
| 10 | `on_kill` | `landing.py:391`（`_apply_damage`） | `actor`, `target`, `dmg` | **击杀者** | 致死伤害落地后；`source is None`（DOT/环境杀）不触发 |
| 11 | `on_death` | `battle.py:625`（`_on_actor_dead`） | `actor`, `target` | 死者 | 所有死亡路径统一在此；**死者的声明仍会执行**（subject 例外） |
| 12 | `dot_tick` | `schedule.py:601`（`_settle_time_effects`） | `actor`, `target`, `key`, `dmg` | 受跳者 | DOT 每一跳 |
| 13 | `dot_calc` | `schedule.py:573`（同上） | `target`, `dot_key`, `dmg`, `mult` | **无**（广播） | DOT 伤害落地**前**的乘区钩子 |
| 14 | `on_act_consume` | `battle.py:488`（`act`） | `actor`, `tag` | 行动者 | 被控跳过行动（`mode="skip"`）时 |
| 15 | `on_hit_consume` | `actions.py:516`（`_consume_hit_buffs`） | `actor`, `key` | 出手者 | 一次性出手 buff 被消费时 |
| 16 | `buff_expire` | `schedule.py:442`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
...
| 21 | `act_done` | `battle.py:570`（`act` 尾部） | `acted`（**不是** `actor`） | **无**（全员广播） | 行动完成；被控跳过不触发 |
| 22 | `phase` | ⚠️ **无引擎点位** | — | — | Boss 阶段转换（上层驱动） |
| 23 | `player_low` | ⚠️ **无引擎点位** | — | — | 玩家低血量（上层驱动） |
| 24 | `pv_broken` | ⚠️ **无引擎点位** | — | — | 破防（上层驱动） |
| 25 | `interrupt` | `landing.py:200`（伤害打断蓄力）/ `effects.py:697`（`act_interrupt` 动词） | `actor`, `target`, `source` | 被打断者 | 读条被打断 |
| 26 | `time_advance` | `schedule.py:402`（`_advance_time`） | `dt`, `now` | **无**（广播） | 时钟推进（结算**之后**广播） |
...
内容侧用法（`battle.py:48-50` 注释）：命令层记账 / 团队技能广播 / 存活同步。
...
（`actions.py:345-429` + `landing.py:26-152` + `battle.py:432-528`）

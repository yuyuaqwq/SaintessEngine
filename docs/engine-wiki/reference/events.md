...
| 16 | `buff_expire` | `schedule.py:239`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
...
| 19 | `taken_calc` | `landing.py:108`（`deal_damage`） | `actor`, `target`, `source`, `dmg`, `mult` | 承伤者 | 承伤修正（承伤方乘区） |
...
（`actions.py:339-423` + `landing.py:25-145` + `battle.py:410-499`）

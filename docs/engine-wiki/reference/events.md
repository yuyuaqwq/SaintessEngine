...
| 16 | `buff_expire` | `schedule.py:564`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
| 17 | `threshold` | `effects.py:455`（`act_apply` 叠层分支） | `actor`, `key`, `value` | 条目持有者 | 叠层数值变化后（「战意满 10 → 狂暴」类） |
...
| 25 | `interrupt` | `landing.py:208`（伤害打断蓄力）/ `effects.py:697`（`act_interrupt` 动词） | `actor`, `target`, `source` | 被打断者 | 读条被打断 |
| 26 | `time_advance` | `schedule.py:407`（`_advance_time`） | `dt`, `now` | **无**（广播） | 时钟推进（结算**之后**广播） |

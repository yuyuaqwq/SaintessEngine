...
| 12 | `dot_tick` | `schedule.py:575`（`_settle_time_effects`） | `actor`, `target`, `key`, `dmg` | 受跳者 | DOT 每一跳 |
| 13 | `dot_calc` | `schedule.py:547`（同上） | `target`, `dot_key`, `dmg`, `mult` | **无**（广播） | DOT 伤害落地**前**的乘区钩子 |
...
| 16 | `buff_expire` | `schedule.py:416`（`_settle_time_effects`） | `actor`, `target`, `key` | 条目持有者 | `effects` 条目到期被删时（原 `buff_expire` 名保留兼容） |
...
| 26 | `time_advance` | `schedule.py:376`（`_advance_time`） | `dt`, `now` | **无**（广播） | 时钟推进（结算**之后**广播） |

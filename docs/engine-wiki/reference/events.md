...
| 17 | `threshold` | `effects.py:432`（`act_apply` 叠层分支） | `actor`, `key`, `value` | 条目持有者 | 叠层数值变化后（「战意满 10 → 狂暴」类） |
...
| 25 | `interrupt` | `landing.py:189`（伤害打断蓄力）/ `effects.py:648`（`act_interrupt` 动词） | `actor`, `target`, `source` | 被打断者 | 读条被打断 |

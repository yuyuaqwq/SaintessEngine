...
| 7 | **物品形状** → 引擎 —— ✅ **已建成**（2026-09-12，[loot.md](loot.md)）：`LootTable`（池 + 策略注册表 + roll/expand/audit）+ `TierTable`（有序档位 + 按等级插值权重表 + 抽档/升档）+ `pick_weighted`/`pick_many`/`roll_range` + `draw_slots`/`count_for`；参考实现侧手写的 590 行掉落引擎 + 四处档位写法 + 词条条数/抽样收敛到引擎，**逐格一致**由冻结比对门禁守护（游戏仓 `tests/test_v184_loot_pools.py` 96 断言/34 413 条 + `test_v184_loot_tiers.py` 3047 断言/4641 条；7 处有意差异逐条登记）；编辑器**`drop_pools` 域 + 池预览 API**（用引擎 `expand/audit` 算，`resolver=None` 不假装知道物品）同步落地 | 搬形状 | — | 掉落/品质/词条规则与具体物品表解耦 |
...
| 7 | 掉落 / 档位 / 词条接入引擎 | `loot` —— ✅ 已接入（2026-09-12，游戏仓本批）|

...
| `cleanse` | bool | ✅ `effects.act_cleanse`（`effects.py:582`） | `True` = 可被净化 |
| `on` | `"caster"` \| `"target"` | ✅ `effects.act_cleanse`（`effects.py:582`，`on=="target"` 也清）；内容侧 `_mech_to_effect` 判 `on_target`（`effects.py:247`） | 效果的默认作用对象。`"target"` = 对敌标记类 |
...
| `on_threshold` | `{层数: {...}}` | ⚠️ **无消费者** | 「满 N 层触发什么」。`threshold` **事件**有引擎点位（`effects.py:432`），但**这张映射表没被读**。目前要靠内容侧监听 `threshold` 自己实现 |
...
| `dot` | — | ⚠️ **无实际消费者** | 只在 `_is_stack_resource` 的判据关键词列表里（`effects.py:277`）。V5 之后 DOT 统一走 `period`，参考实现 78 个 key 里**无一条**使用 |

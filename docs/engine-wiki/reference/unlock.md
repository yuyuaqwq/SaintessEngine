...
> **归属**：本能力**不在引擎里**（2026-09-23 起）—— 它在扩展包 `extends/ext_life/`（`unlock/`，E4，2026-09-21）· **形状，零 hook**。
> 数据包要用它：`game.json` 里写 `"depends": ["ext_life"]`。
> 引擎侧只剩通用件，见 `../architecture/boundaries.md`。
> **为什么建**：当初引擎里已有 `run` / `quest` / `presence`（现分别在 `extends/ext_world/`、`extends/ext_quest/`、`extends/ext_social/`），但**都不是解锁闸门** ——
...
本形状**不出现**「阶段 / 地图 / 指令 / 系统」这些字面量：
`targets[].kind` 的取值集合**由包声明**（包的 `unlock_kinds` 域），
本形状只当**不透明字符串**做等值匹配 —— 不猜"这个 id 属于哪种目标"。
...
裸 id 会逼形状层猜它属于哪一种 —— 不许猜。二元组把语义留给内容侧，本形状只做等值匹配。

...
（`_check_side_end` 里 `alive[0] == "player"` → `result="victory"`，`battle.py:663`）。
...
直到撞上**下一个 ct 最小的人控 actor**（`schedule.advance`，`schedule.py:286`）。
单玩家场景 `who` 通常仍是自己。
...
- 胜负判定在 `_check_side_end`（`battle.py:657`）：存活阵营数 ≤ 1 → 置 `result`；
  `alive[0] == "player"` → `"victory"`，否则 `"defeat"`；全灭 → `"defeat"`。
- `"fled"` 只由 `Battle._do_flee`（`battle.py:603`）写。

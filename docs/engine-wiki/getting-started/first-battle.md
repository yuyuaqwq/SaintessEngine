...
- `kind` 只是**数据标签**（`"player"` / `"monster"` / 你自己的词），引擎不按它分支；
  真正决定「谁是人控」的是 `human_controlled`（`Battle.focus()` 只看它，`battle.py:181`）。
...
`Battle.__init__`（`battle.py:32`）做的事，按顺序：

1. 把 `sides` 拷成 `self.sides`（dict，值是 list）—— `battle.py:60-63`
2. `hostile_map` 缺省 → 之后由 `hostile_sides()` 推「除自己外全部阵营」（`actors.py:193`）
3. 建技能索引 `actor["_skill_index"]`（`_index_skills` → `_index_one_actor`，`battle.py:164/117`）
4. **播种初始 ct**（`_seed_ct_one`，`battle.py:90`）：`ct = action_time(聚合 spd)`
   —— 快者先手、开局第一动也按速度排（`schedule.initial_ct`，`schedule.py:37`）

`sides` 的键名由你定；引擎唯一硬编码的约定是 **`"player"`** 这个键名
（`_check_side_end` 里 `alive[0] == "player"` → `result="victory"`，`battle.py:545`）。
...
`human_act`（`battle.py:234`）的返回是三元组：
...
```python
b.auto_run([])                       # battle.py:292，全自动跑到 result != None
print(b.result, b.winner_side)       # victory / player
```

- `auto_run` 里人控 actor 也走普攻（`battle.py:302`），适合测试与仿真。
- 胜负判定在 `_check_side_end`（`battle.py:529`）：存活阵营数 ≤ 1 → 置 `result`；
  `alive[0] == "player"` → `"victory"`，否则 `"defeat"`；全灭 → `"defeat"`。
- `"fled"` 只由 `Battle._do_flee`（`battle.py:488`）写。
...
| `Battle.action_override` | 构造参数，`act()` 里非内置动作时调用（`battle.py:455`） | 接管 `use_item` 之类的自定义行动 |
...
`add_actor(actor, side, front=False)`（`battle.py:207`）：入 sides → 建技能索引 →

...
翻译规则（`effects.resolve_actions`，`effects.py:140`）：
...
| 表里没有、也不是动词 | **静默跳过**（`effects.py:200-201` `continue`） ← 最常见的「我的效果没生效」原因 |

参数合并语义（`_merge_params`，`effects.py:160`）：**调用方显式给的参数优先**，
...
`attack_hit` 的触发点在 `_single_target_pipeline` 尾部（`actions.py:488`）：
...
`_apply_hit_effects`（`actions.py:525`）执行。分派判据见 `_mech_to_effect`

...
def t2_positive(body, tpat):
 ...
 #   ★ 2026-09-19 C 档 21b（B-2 第 7 片第 2 小片：生活「商店 · 买卖」35 新键）⇒ 2173 → 2208。
    ...
    # ★ C 档 36a（B-2 第 29 片）：`content/cmds_world.py::quest_view` 句壳入表
    #   （27 处 / 23 新键，新分类 任务面板）⇒ 3103 → 3126。
    ...
    check("包内文案条数锚点 == 3194（条数变了就同步更新本门禁的锚点）", len(body) == 3194, len(body))
...
def t3_chinese_keys_revive(body, tpat):
    ...
    #   ★ 2026-09-19 C 档 PRE2（并行批 2）：路 A 的 4 个中文键
    #     （instance.面板_站位_敌方 / _我方 / _防御 / _精英）⇒ 165 → 169。
    check("含中文的 key 数 == 169（域研究口径）", len(cn) == 169, len(cn))
    rx = re.compile(tpat)

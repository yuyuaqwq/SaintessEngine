...
from saintess_engine.run import (DENY, PASS, SKIP, Admission, Progress,         # noqa: E402
                                 Roster, Rule, Verdict)
...
def t8_zero_knowledge():
    ...
    bad = []
    for root, _dirs, files in os.walk(os.path.join(ROOT, "saintess_engine", "run")):
        for fn in files:
...
def t9_facade():
    print("\n[9] 门面：子模块与公开符号")
    sub = importlib.import_module("saintess_engine.run")
    check("`saintess_engine.run` 是同一模块对象", SE.run is sub)
    for sym in ("Admission", "Rule", "Verdict", "Progress", "Roster"):
        check(f"门面转出 {sym}", hasattr(SE, sym) and getattr(SE, sym) is getattr(sub, sym))
    check("`from saintess_engine import run, Admission, Rule, Progress, Roster` 可用",
          all(x in SE.__all__ for x in ("run", "Admission", "Rule", "Progress", "Roster")))

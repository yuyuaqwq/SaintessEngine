...
import saintess_engine.presence as PRESENCE                              # noqa: E402
from saintess_engine.presence import (Lookup, Presence, cooldown_ok,     # noqa: E402
                                      day_hit, day_slot, guarded_roll,
                                      merge_tables, minutes_left)

PRESENCE_DIR = os.path.join(ROOT, "saintess_engine", "presence")
...
def t9_zero_knowledge():
    ...
    check("绝对 import 全部落在标准库（可分发性）",
          roots <= set(sys.stdlib_module_names), f"roots={sorted(roots)}")

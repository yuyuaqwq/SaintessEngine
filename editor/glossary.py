...
_EFFECT_RULES = {
    ...
    "period.pct_cur_boss": {"zh": "Boss 档当前生命比例", "note": "✅ 标签档覆盖 pct_cur_hp（与 `period.pct_boss` 同口径：看该周期声明的 `trait_tags`）。", "ref": ("reference/effect-rules.md", "pct_cur_boss")},
    ...
    "period.boss_pct_mult": {"zh": "周期 boss 折扣", "note": "✅ 引擎消费：**命中该周期声明标签**（`trait_tags`）的目标的百分比段折扣系数（条目级 `pct_boss` 未声明时才用，二者不叠乘）。引擎只问「名单里有没有命中」，不认标签叫什么。", "ref": ("reference/effect-rules.md", "boss_pct_mult")},

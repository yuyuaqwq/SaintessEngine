...
两个**并列的包**，纯度契约相同（都只依赖相对导入 + 标准库，都可整包拷走）：

| 包 | 定位 |
|---|---|
| `saintess_engine/` | **协议层**：事件总线 / 效果叠层 / 声明表 / CTB 调度 / 落地结算 / 存档 —— 极窄且稳定 |
| `saintess_kit/` | **运行时骨架层**：存储骨架 / 命令框架 / 事件 / 时钟 / 容器 / 会话适配 —— 长期演进 |

```
saintess_engine/            # 引擎包（协议层，可整包拷走）
...
saintess_kit/               # 运行时骨架包（可整包拷走）
  store/            #   SQLite 骨架：连接/锁/事务 + 建表注册 + 列迁移 + Repository
  events/           #   领域事件总线：注册序执行 / 段落空行 / 未知事件策略 / 异常容忍
...
**③ 想给框架加通用能力**（别人也能用） —— 改 `saintess_engine/`（引擎语义）或
`saintess_kit/`（运行时骨架），跑本仓库测试；机制动作请优先放自己游戏包的 `mech/`
（那才是「外挂」该待的地方）。

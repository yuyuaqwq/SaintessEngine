...
import ext_social.membership as MEMBERSHIP                        # noqa: E402
from ext_social.membership import (Applications, Contribution,   # noqa: E402
...
def _membership_dir():
    return os.path.join(ROOT, "extends", "ext_social", "membership")
...
def t1_single_roster():
    ...
    hits = []
    for root, dirs, files in os.walk(os.path.join(ROOT, "extends/ext_world")):
    ...
    check("★ 只有 run/roster.py 定义 Roster", files_hit == ["extends/ext_world/run/roster.py"],
          str(hits))
